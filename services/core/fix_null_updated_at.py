...
async def fix_null_updated_at():
    ...
    async with AsyncSessionLocal() as db:
        ...
        logger.info(f"Found {len(goals)} goals with NULL updated_at")

        if not goals:
            logger.info("✅ No goals need fixing")
            return
        ...
        for goal in goals:
            if goal.created_at:
                ...
                logger.info(f"✅ Fixed goal {goal.id}: {goal.title[:50]}")
            else:
                now = datetime.utcnow()
                goal.updated_at = now
                goal.created_at = now
                logger.info(f"✅ Fixed goal {goal.id}: {goal.title[:50]} (set to now)")
        ...
        if remaining:
            logger.info(f"⚠️  Still {len(remaining)} goals with NULL updated_at")
        else:
            logger.info("✅ All goals now have updated_at set")
