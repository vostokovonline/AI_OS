...
class RootReducer:
    ...
    - Historical: show all events including compensated
    - Effective: mask compensated transactions
    """
    ...
    REDUCER_MAP = {
        ...
        "transaction_committed": ("transaction", "commit"),
        "transaction_compensated": ("transaction", "compensate"),
    }
    
    # Track compensated transactions for effective mode
    _compensated_transactions: set = set()
    
    @classmethod
    def set_effective_mode(cls, effective: bool = True, compensated_ids: set = None):
        """Set replay mode and provide compensated transaction IDs"""
        if effective and compensated_ids:
            cls._compensated_transactions = compensated_ids
        elif not effective:
            cls._compensated_transactions = set()
    ...
    def apply(state: Dict[str, Any], event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif reducer_name == "transaction":
            return RootReducer._apply_transaction_event(state, enriched_event)
    ...
    @staticmethod
    def _apply_transaction_event(state: Dict[str, Any], event: Dict[str, Any]) -> Dict[str, Any]:
        """
        Handle transaction events.
        
        For compensation:
        - Record the compensation in state
        - Mark affected transaction as compensated
        """
        new_state = deepcopy(state)
        ...
        if op == "compensate":
            payload = event.get("payload", {})
            transaction_id = payload.get("original_transaction_id", event.get("target_id"))
            
            new_state["transactions"][transaction_id] = {
                "status": "compensated",
                "compensated_at": event.get("version", 0),
                "reason": payload.get("reason", "manual_compensation")
            }
            
            # Track for effective replay
            if transaction_id not in RootReducer._compensated_transactions:
                RootReducer._compensated_transactions.add(transaction_id)
